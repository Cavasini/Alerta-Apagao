...
public class AuthenticationController {
    ...
    public ResponseEntity register(@Valid @RequestBody RegisterDTO data){
        if(this.userRepository.findByPhoneNumber(UsersUtils.validatePhoneNumber(data.phoneNumber())) != null) return ResponseEntity.badRequest().body("PhoneNumber already used");
        if(this.userRepository.findByName(data.login()) != null) return ResponseEntity.badRequest().body("User already exists");
